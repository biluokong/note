...
* 集合中要添加引用类型，只需要把`<value>`标签改成`<ref>`标签，这种方式用的比较少## Spring_day02
...
- 使用@Value注解时，如果配置文件没有值时会报错，需要指定初始值。

  ```java
  @Value("${spring.redis.password:}")	// 指定初始值为空字符串
  @Value("${spring.redis.password:2206}")	// 指定初始值为2206
  ```
...
| 属性 | value（默认）：要注入的属性值（如${注入属性值:初始值}） |
...
| 属性 | classes：核心配置类，可以使用数组的格式设定加载多个配置类<br/>locations:配置文件，可以使用数组的格式设定加载多个配置文件名称 |## Spring_day03
