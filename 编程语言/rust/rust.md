...
2. 配置系统环境变量（按win键直接搜索系统环境变量打开即可），在`path`环境变量中新增一条，将`D:\dev\rust\mingw64\bin`
...
在 Rust 中，权限修饰符（或可见性修饰符）用于控制模块、类型、函数、方法等项目的可见性。

> **crate** 是代码组织和分发的基本单位，可以理解为一个独立的编译单元或项目。它是 Rust 生态中最核心的概念之一，类似于其他语言中的“库（Library）”或“包（Package）”，但功能更全面。
>
> | **术语**      | **Rust 中的含义**                                            |
> | ------------- | ------------------------------------------------------------ |
> | **Crate**     | 独立的编译单元（库或二进制程序）。                           |
> | **Package**   | Cargo 的项目管理单元（包含一个 `Cargo.toml` 和至少一个 crate）。 |
> | **Module**    | 组织 crate 内部代码的结构（如 `mod foo { ... }`）。          |
> | **Workspace** | 多个相关 crate 的集合（共享 `Cargo.toml` 的 `[workspace]` 配置）。 |

| **修饰符**           | **语法**                    | **可见范围**                                          | **典型使用场景**                                             |
| -------------------- | --------------------------- | ----------------------------------------------------- | ------------------------------------------------------------ |
| **默认（无修饰符）** | `fn foo()`                  | 仅在当前模块内可见（私有）                            | 模块内部使用的辅助函数或数据结构                             |
| **`pub`**            | `pub fn foo()`              | 对整个 crate 和外部代码可见（公开）                   | 需要暴露给 crate 用户或跨模块调用的公共 API                  |
| **`pub(crate)`**     | `pub(crate) fn foo()`       | 当前 crate 内部可见（对外部隐藏）                     | 跨模块共享代码，但不想暴露给 crate 的用户（如内部工具函数）  |
| **`pub(super)`**     | `pub(super) fn foo()`       | 当前模块和直接父模块可见                              | 子模块中需要被父模块访问，但对其他模块隐藏的逻辑（如父模块聚合子模块功能） |
| **`pub(in path)`**   | `pub(in crate::a) fn foo()` | 仅对指定路径的模块可见（如 `crate::a` 或 `super::b`） | 精细控制可见性，限制到特定模块（较少使用，适合复杂项目结构） |
...
**泛型常量（Generic Constants）** 是 Rust 1.70 版本（2023 年 6 月发布）引入的新特性，它允许在泛型参数中直接使用常量表达式（如数值、布尔值、字符等），从而在编译时生成更高效的代码。这一特性扩展了 Rust 泛型的能力，使其能够处理更多编译时已知的常量值。
...
在 Rust 1.70 之前，泛型参数只能是类型（`T`）或生命周期（`'a`），无法直接使用常量值。例如：
...
如果需要基于常量值（如数组长度）进行泛型编程，通常需要借助 **关联常量（Associated Constants）** 或 **宏（Macros）**，但这些方法不够灵活或直观。

Rust 1.70 允许在泛型参数中直接使用常量表达式（`const`），例如：
...
这里 `N` 是一个泛型常量参数，表示数组的长度，必须在编译时已知。

> 语法：`<const PARAM: TYPE>`
>
> - `PARAM` 是常量参数名（通常大写，如 `N`、`SIZE`）。
> - `TYPE` 是常量的类型（如 `usize`、`u32`、`bool`、`char` 等）。

**泛型常量 vs 关联常量：**

| **特性**     | **泛型常量（Generic Constants）**       | **关联常量（Associated Constants）**         |
| ------------ | --------------------------------------- | -------------------------------------------- |
| **定义位置** | 泛型参数（`<const N: usize>`）          | `trait` 或 `impl` 块中（`const NAME: TYPE`） |
| **用途**     | 参数化常量值（如数组长度）              | 定义类型关联的常量（如 `i32::MAX`）          |
| **示例**     | `fn foo<const N: usize>(arr: [i32; N])` | `trait Bar { const VALUE: u32; }`            |
