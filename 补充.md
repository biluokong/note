...
- 字段属性是逗号分隔的字符串，例如字段名`ancestors`，其属性值是`3,400,23`，怎么查询出该字段属性值有`23`的记录。
...
WHERE db = 'wf_hotel_data' AND info like '%ht_hotel%';
# 上面查询结果中有进程id，通过kill命令可以杀手指定进程
kill [id];
...
有时`Logback`的默认高亮配置不符合自己的需求，可以自定义转换器。

> 更简单的方法是在提高`{}`来指定，但某些旧版本不支持。
>
> ~~~xml
> <property name="log.pattern" value="%d{yyyy-MM-dd HH:mm:ss.SSS} [%thread] %highlight(%-5level){} %logger{{FATAL=red, ERROR=red, WARN=yellow, INFO=green, DEBUG=cyan, TRACE=blue} [%method,%line] - %msg%n"/>
> ~~~

1. 继承默认配置的`HighlightingCompositeConverter`自定义日志级别和对应的高亮颜色。
...
2. 在`logback.xml`或`logback-spring.xml`文件里指定转换器。

~~~xml
<configuration>
  <!-- 显式定义颜色映射 -->
  <conversionRule conversionWord="highlight" converterClass="com.manage.config.CustomHighlightingConverter"/>

  <!-- 日志输出格式 -->
  <property name="log.pattern" value="%d{yyyy-MM-dd HH:mm:ss.SSS} [%thread] %highlight(%-5level %logger{20} [%method,%line] - %msg%n)"/>
</configuration>
~~~
...
public class ApiUtils {
  ...
  public static final RestTemplate CLIENT;
  ...
  /**
     * 发送post请求：body为json格式
     */
  public static String post(String url, JSONObject dataJson) {
    return post(url, dataJson, new MediaType("application", "json", StandardCharsets.UTF_8), null);
  }

  /**
     * 指定token发送post请求：body为json格式
     */
  public static String post(String url, JSONObject dataJson, String token) {
    return post(url, dataJson, new MediaType("application", "json", StandardCharsets.UTF_8), token);
  }

  /**
     * 发送post请求
     */
  public static String post(String url, JSONObject dataJson, MediaType contentType, String token) {
    // 1.构造请求头
    ...
    // 2.构造请求体
    HttpEntity<JSONObject> entity = new HttpEntity<>(dataJson, headers);

    // 3.发送请求
    return CLIENT.postForEntity(url, entity, String.class).getBody();
  }
}
~~~
...
~~~sh
# 统计一年的代码改动行数
git log --since="Jan 1 2025" --until="Dec 31 2025" --pretty=tformat: --numstat | awk '{ add += $1; subs += $2; loc += $1 - $2 } END { printf "添加行数: %s, 删除行数: %s, 净改动: %s\n", add, subs, loc }'

git log --since="2025-01-01" --until="2025-12-31" --pretty=tformat: --numstat | awk '{ add += $1; subs += $2; loc += $1 - $2 } END { printf "添加行数: %s, 删除行数: %s, 净改动: %s\n", add, subs, loc }'

# 加上作者的过滤项
git log --since="Jan 1 2023" --until="Dec 31 2023" --author="作者名" --pretty=tformat: --numstat | awk '{ add += $1; subs += $2; loc += $1 - $2 } END { printf "添加行数: %s, 删除行数: %s, 净改动: %s\n", add, subs, loc }'

# 指定文件类型
git -c diff.renameLimit=1338 log --since="Jan 1 2025" --until="Dec 31 2025" --pretty=tformat: --numstat | awk '$3 ~ /\.java/ && $1 != "-" { add += $1; subs += $2; loc += $1 - $2 } END { printf "添加行数: %s, 删除行数: %s, 净改动: %s\n", add, subs, loc }'


# 获取 2025 年初的最后一个提交
#start_commit=$(git rev-list -n 1 --before="Jan 1 2025" HEAD)
# 获取最早的提交（仓库创建时的提交）
start_commit=$(git rev-list --max-parents=0 HEAD)
# 获取 2025 年末的最后一个提交
end_commit=$(git rev-list -n 1 --until="Dec 31 2025" HEAD)
# 获取两次提交之间的代码行修改数
git diff --shortstat "$start_commit" "$end_commit" -- "*.java"
~~~
