...
mp的`BaseMapper`只提供了简单的增删改查方法，一些批量操作的方法没有提供，只能通过调用继承`IService`的service类才能使用。另一种方法是通过扩展`BaseMapper`来使其获取新的功能。
...
> 实际通过前面二步，生成的`BaseMapper`代理类已经有了扩展的方法，这一步了是为了让接口类也拥有扩展方法。
