...
（Single Responsibility Principle, SRP）定义：一个类应该有且仅有一个引起它变化的原因，否则类应该被拆分。
...
单一职责原则的核心就是控制类的粒度大小、将对象解耦、提高其内聚性。具体来说，其作用如下：
...
（Open Closed Principle，OCP）定义：**对扩展开放，对修改关闭**。在程序需要进行拓展的时候，不能去修改原有的代码，实现一个热插拔的效果。简言之，是为了使程序的扩展性好，易于维护和升级。
...
（Dependence Inversion Principle，DIP）定义：高层模块不应该依赖低层模块，两者都应该依赖其抽象；抽象不应该依赖细节，细节应该依赖抽象。简单的说就是要求对抽象进行编程，不要对实现进行编程，这样就降低了客户与实现模块间的耦合。
...
（Interface Segregation Principle，ISP）定义：客户端不应该被迫依赖于它不使用的方法；一个类对另一个类的依赖应该建立在最小的接口上。
...
合成复用原则（Composite Reuse Principle，CRP）又叫(组合)合成/聚合复用原则（Composition/Aggregate Reuse Principle，C/ARP）是指：尽量先使用组合或者聚合等关联关系来实现，其次才考虑使用继承关系来实现。
...
​	stu对象和stu1对象是同一个对象，就会产生将stu1对象中name属性值改为“李四”，两个Citation（奖状）对象中显示的都是李四。这就是浅克隆的效果，对具体原型类（Citation）中的引用类型的属性进行引用的复制。这种情况需要使用深克隆，而进行深克隆有两种方法：

1、引用类型也调用其clone方法获取。修改clone方法，代码如下：

```java
//深克隆，在clone方法中调用引用类型的clone方法拷贝引用类型
    public CitationU clone() throws CloneNotSupportedException {
        CitationU clone = (CitationU) super.clone();
        clone.student = (Student) student.clone();
        return clone;
    }
```

2、使用序列化反序列化。需要使用对象流，代码如下：
...
CGLIB是一个功能强大，高性能的代码生成包。它为没有实现接口的类提供继承方式的代理，为JDK的动态代理提供了很好的补充。

注意：cglib代理的类不能为final修饰的类。
...
它可以在Java程序运行时定义一个新的类，并加载到JVM中；还可以在JVM加载时修改一个类文件。Javassist使用户不必关心字节码相关的规范也是可以编辑类文件的。

- 动态创建一个类的基本步骤为：获取类池 -> 制造类 -> 获取接口 -> 实现接口 -> 创建属性 -> 将属性添加到类中 -> 创建方法 -> 将方法添加到类中

  <img src="img/javassist_01.png" style="zoom:80%;" />

- 实现接口用addInterface方法，设置父类使用setSuperclass方法。

- 用javassist实现动态代理可以使用继承，也可以使用实现接口。只需要在创建类时设置其父类或实现相应接口。
...
实现方式：对象适配器模式可釆用将现有组件库中已经实现的组件引入适配器类中，该类同时实现当前系统的业务接口。即采用聚合的关系，而不采用继承的关系。
...
> 注意：还有一个适配器模式是接口适配器模式。当不希望实现一个接口中所有的方法时，可以创建一个抽象类Adapter ，实现所有方法。而此时我们只需要继承该抽象类即可，这样子类就能有选择的覆盖父类的某些方法。
...
<img src="img/装饰者模式.png" style="zoom:80%;" />
...
​	将抽象与实现分离，使它们可以独立变化。它是用聚合/组合关系代替继承关系来实现，从而降低了抽象和实现这两个可变维度的耦合度。
...
public class GroupLeader extends Handler {
    public GroupLeader() {
        ...
        //若上级领导固定，直接设置即可
        super.setNextHandle(new Manager());
    }
...
public class Manager extends Handler {
    public Manager() {
        ...
        //若上级领导固定，直接设置即可
        super.setNextHandle(new GeneralManager());
    }
...
public class Client {
    public static void main(String[] args) {
        ...
        //也可以在这里设置上级领导，是因为可以根据实际需求来更改设置，如果实际中上级领导人都是固定的，则可以移到领导实现类中。
...
<img src="img/观察者模式2.png" style="zoom:80%;" />
...
<img src="img/迭代器模式2.png" style="zoom:90%;" />
