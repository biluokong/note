...
> synchronized、Lock、Semaphore、CountDownLatch、
>
> CyclicBarrier
...
> - 死锁产生条件
> 	- **互斥条件（Mutual Exclusion）**：至少一个资源必须处于非共享模式，即一次只能被一个进程占用。也就是说，当一个资源被一个进程使用时，其他进程无法同时使用它。
> 	- **持有并等待条件（Hold and Wait）**：进程必须占有至少一个资源，并且在等待获取其他进程占有的资源。这意味着一个进程已经持有一些资源，同时它还在等待其他进程释放的资源。
> 	- **不可剥夺条件（No Preemption）**：进程已获得的资源在未使用完之前，不能被其他进程强行剥夺，只能由持有资源的进程释放。一旦一个进程持有了某个资源，这个资源就不能被其他进程强行拿走，除非该进程自己释放它。
> 	- **循环等待条件（Circular Wait）**：多个进程之间形成一个等待环路，每个进程等待下一个进程所占有的资源。也就是说，存在一个循环链，进程A等待进程B的资源，进程B等待进程C的资源，而进程C又等待进程A的资源，这样就形成了一个闭环。
> - 解决已发生的死锁
> 	- 资源剥夺
> 	- 撤销进程
> - 预防方法
> 	- 避免嵌套锁：让各线程必须按某种顺序获取锁（资源顺序分配）。
> 	- 超时机制：获取锁时设置超时时间。
