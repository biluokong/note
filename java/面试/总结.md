...
- String：不可变类型，通过不可变保证线程安全
- StringBuffer：可变类型，方法上加锁保证线程安全
- StringBuilder：可变类型，线程不安全，效率高

底层类型都是char数组，String类型JDK9后使用byte数组
...
![image-20250227135908032](img/image-20250227135908032.png)
...
> 注：由于跳页时不知道指定页的第一条数据的id，所以这种方式不支持跳页；但现在很多业务都使用瀑布流方式，所有此方法很常用
...
1. **简化开发**：通过提供丰富的组件和库，Spring大大简化了Java应用程序的开发过程。
2. **降低耦合度**：Spring的IoC容器帮助管理对象及其依赖关系，降低了代码之间的耦合度，提高了代码的可维护性。
3. **增强可测试性**：Spring支持依赖注入和Mock对象，使得单元测试更加容易实现，提高了代码的可测试性。
4. **支持AOP**：通过AOP，Spring允许程序员在不修改业务逻辑代码的情况下添加额外的功能，如日志记录、事务管理等。
5. **整合其他技术**：Spring与多种技术和框架（如Hibernate、MyBatis等）无缝集成，为程序员提供了更多的选择和灵活性。
6. **清晰的架构**：Spring鼓励使用清晰的分层架构，使得代码结构更加清晰，易于理解和维护。

### spring模块

- spring-core：提供IoC和DI的基础功能
- spring-beans：实现BeanFactory接口，支持对象的创建、配置和管理
- spring-context：扩展BeanFactory，提供国际化、事件传播等功能
- spring-expression：支持SpEL
- spring-jdbc：提供JDBC的抽象层
- spring-orm：支持ORM框架
- spring-tx：提供事务管理
- spring-web (基本web支持)、webmvc (实现mvc框架)、websocket
- spring-aop/aspects：实现AOP功能，提供基于注解的AOP实现
- spring-test：提供测试工具和API
...
> 生命周期的主要阶段：实例化、属性赋值(依赖注入)、初始化、销毁
...
> 注：springboot3.0后，spring.factories文件被废弃，换成了`META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports`文件
...
> - 负载均衡（Load Balancing）
>
> 注：springcloud2020版本后，移除了Ribbon，换成了`SpringCloud-LoadBalancer`
...
> 	~~~
>
> - sentinel：@SentinelResource(value=“资源名”, blockHandler=“函数名”, fallback=“函数名”)
...
![image-20210724174424070](img/image-20210724174424070.png)
...
- 拆分为多个单一队列，每个队列对应一个消费者；但这样会造成吞吐量下降，可以在消费者内部采用多线程的方式去消费。
...
- 开闭原则（Open Closed Principle，OCP）：**对扩展开放，对修改关闭**。在程序需要进行拓展的时候，不能去修改原有的代码，实现一个热插拔的效果。简言之，是为了使程序的扩展性好，易于维护和升级。
- 里氏代换原则（Liskov Substitution Principle，LSP）：任何基类可以出现的地方，子类一定可以出现。通俗理解：子类可以扩展父类的功能，但不能改变父类原有的功能。换句话说，子类继承父类时，除添加新的方法完成新增功能外，尽量不要重写父类的方法。
- 依赖倒转原则（Dependence Inversion Principle，DIP）：高层模块不应该依赖低层模块，两者都应该依赖其抽象；抽象不应该依赖细节，细节应该依赖抽象。简单的说就是要求对抽象进行编程，不要对实现进行编程，这样就降低了客户与实现模块间的耦合。
- 接口隔离原则（Interface Segregation Principle，ISP）：客户端不应该被迫依赖于它不使用的方法；一个类对另一个类的依赖应该建立在最小的接口上。
- 迪米特法则（Law of Demeter，LoD）又叫最少知识原则：只和你的直接朋友交谈，不跟“陌生人”说话；如果两个软件实体无须直接通信，那么就不应当发生直接的相互调用，可以通过第三方转发该调用。其目的是降低类之间的耦合度，提高模块的相对独立性。
...
类图中属性/方法名称前加的加号和减号表示了这个属性/方法的可见性，UML类图中表示可见性的符号有三种：
...
- 关联关系：对象之间的一种引用关系（成员变量）
  - 单向关联：用一个带箭头的实线表示
  - 双向关联：用一个不带箭头的直线表示
  - 自关联：用一个带有箭头且指向自身的线表示
- 聚合关系：强关联关系，是整体和部分之间的关系，但部分可以脱离整体存在；聚合关系可以用带空心菱形的实线来表示（菱形指向整体）
- 组合关系：更强烈的聚合关系，部分不能脱离整体存在；用带实心菱形的实线来表示（菱形指向整体）
- 依赖关系：使用关系，耦合度最弱的关联方式；在代码中，某个类的方法通过局部变量、方法的参数或者对静态方法的调用来访问另一个类（被依赖类）中的某些方法来完成一些职责；用带箭头的虚线来表示
- 继承关系：耦合度最大的一种关系，一般与特殊、父类与子类的关系；泛化关系用带空心三角箭头的实线来表示
- 实现关系：接口与实现类之间的关系；用带空心三角箭头的虚线来表示
...
1. 单例模式
   - 饿汉式：类加载就会导致该单实例对象被创建（静态变量/静态代码块/枚举类方式）
   - 懒汉式：类加载不会导致该单实例对象被创建，而是首次使用该对象时才会创建（双重检查锁/静态内部类式）
2. 工厂方法模式
3. 抽象工厂模式
4. 原型模式
5. 建造者模式
