...
重新一个不存在的数据，mysql查询不到数据，就不会写入缓存，导致每次请求都查询数据库，这可能导致DB挂掉。
...
- 方法1：互斥锁（分布式锁）
	...
	- 特点：强一致性，但性能差
- 方法2：逻辑过期（或不过期）
	- 查询缓存时，发现数据已经逻辑过期就直接返回就数据，并获取互斥锁后，开启新的线程去更新缓存；其他发现数据逻辑过期的线程在获取锁失败后，也直接返回旧数据。
	- 特点：最终一致性，高可用，性能好

#### 缓存雪崩

同一时段大量key同时过期或redis服务器宕机，导致大量请求到达数据库，带来巨大压力，可能是数据库挂掉。
...
- 利用Redis集群提高服务的可用性
	- 哨兵模式
	- 集群模式
- 会缓存业务添加降级限流策略（降级可作为系统的保底策略，适用于穿透、击穿、雪崩）
	- nginx
...
- 该方法不能完全避免有脏数据的风险；不加锁性能好；小概率产生脏数据，延迟越久概率越小

#### 读写锁

由于一般放入缓存的数据多少读多写少，此时可以用redisson提供的读写锁来保证数据的同步。
...
Redis Database Backup file（Redis数据备份文件），也叫Redis数据快照。简单来说就是把内存中的数据都记录到磁盘中，但Redis示例故障重启后，从磁盘读取快照文件，恢复数据。
...
- 定期删除：每隔一段时间，就对一些key进行检查，删除里面过期的key
	...
	- 优点：可以通过限制删除操作执行的时长和频率来减少删除操作对CPU的影响。可能有效释放内存。
	- 缺点：难以确定删除操作执行的时长和频率。
...
redisson加锁、设置过期时间等操作都是具有Lua脚本完成，可以保证redis多条命令执行的原子性。
...
数据读写：分片集群引入了哈希槽，集群总共有16384个哈希槽，每个key通过CRC16校验后对16384取模来决定放哪个槽，集群的每个master负责一部分hash槽。
...
2. DispatcherServlet收到请求调用HandleMapping（处理器映射器）。
3. HandleMapping找到具体的处理器，生成处理器对象及处理器拦截器(如果有)，再一起返回给DispatcherServlet。
...
5. HandlerAdapter经过适配调用具体的适配器（Handler/Controller）。
...
1. springboot项目引导类上有一个注解@SpringBootApplication，这个注解对三个注解进行了封装：
	...
	- @ComponentScan
2. 其中@EnableAutoConfiguration是实现自动化配置的核心注解。该注解通过@Import注解导入对应的配置选择器。（它会读取该项目和该项目引用的jar包的classpath路径下META-INF/spring.factories文件中所配置的类的全类名。这些配置类中所定义的bean会根据条件注解所指定的条件来决定是否将该配置类定义的bean导入到spring容器中）
...
4. 操作数据库接口：Executor执行器，同时负责查询缓存的维护。
...
基于PerpetualCache的HashMap本地缓存，器存储作用域为session，但session进行flush或close之后，该session的cache就将被清空，默认打开了一级缓存。
...
- 异：
	...
	- 支持服务列表变更的消息推送模式，服务列表更新更及时（既注册pull，有注册push）。
...
![image-20210724174424070](C:\Users\797799421\Desktop\笔记\中间件和微服务框架\黑马\springcloud微服务栈\2022\高级篇\分布式事务\assets\image-20210724174424070.png)
...
![image-20210724175327511](C:\Users\797799421\Desktop\笔记\中间件和微服务框架\黑马\springcloud微服务栈\2022\高级篇\分布式事务\assets\image-20210724175327511.png)
...
![image-20210724182937713](C:\Users\797799421\Desktop\笔记\中间件和微服务框架\黑马\springcloud微服务栈\2022\高级篇\分布式事务\assets\image-20210724182937713.png)
...
| PUT      | 更新操作，如果是以决定值更新，是幂等的；如果是通过增量方式更新，则不是幂等的 |
...
当生产者发送消息的速度超过了消费者处理消息的速度，就会导致队列中的消息堆积，直到队列存储消息到达上限。之后发送的消息就会成为死信，可能会被丢弃，则就是消息堆积问题。
...
| acks=-1  | 只有当所有参与赋值的节点全部收到消息时，生产者才会收到一个来自服务器的成功响应。 |
...
- 生产者发送消息到broker丢失
	...
	- 失败重试，参数配置，可以设置重试次数。
- 消息在broker中存储丢失
	- 发送确认acks，选择all(-1)，让所有的副本都参与保存数据后的确认。
- 消费者从broker接收信息丢失
	- 关闭自动提交偏移量，开启手动提交偏移量。
	- 提交方式最后是：手动 + 异步提交。
