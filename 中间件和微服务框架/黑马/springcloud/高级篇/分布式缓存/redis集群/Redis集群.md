...
注：或者用新conf文件的配置覆盖部分配置

```sh
touch redis_7001.conf
# 在创建的conf文件中添加以下配置：
include /tmp/7001/redis.conf	# 指定读取旧配置的文件
# 运行多个redis服务时，需要指定不同的pid文件和端口
pidfile /tmp/7001/redis_7001.pid	
port 7001
dir /tmp/7001/	# 备份文件存放目录
dbfilename dump7001.rdb	# rdb备份文件名
# redis实例的声明 IP
replica-announce-ip 192.168.30.3
# 然后指定不同配置文件运行redis-server即可
```
...
# 方式一：永久生效。在配置文件中加
slaveof ip port
# 如果master设置了密码，配置文件中加
masterauth 3238
# 方式二：临时生效
slaveof 192.168.150.101 7001 [masterauth 密码]
```
