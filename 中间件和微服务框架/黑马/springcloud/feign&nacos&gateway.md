...
注：变种是定义了FeignClient后，提供方继承该FeignClient类实现功能。
...
public class UserController implements UserClient {
    @GetMapping("/user/{id}")
    public User findById(@PathVariable("id") Lond id) {
        ....
    }
}
~~~
...
每个服务中拆分为三个模块：公共dto模块、feign接口模块、业务模块。其他service想要使用该服务的接口，依赖该服务下的接口模块和dto模块即可。

此方式服务之间的耦合度很低，feign接口有该服务编写者维护，高内聚；但此种方式会增加微服务架构的复杂度。

## 2.5.日志

只有在FeignClient所在包的日志级别为DEBUG时，才会输出日志。而且它自身又对日志分为了四个级别。
