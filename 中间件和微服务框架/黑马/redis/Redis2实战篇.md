...
​		2、删除缓存的速度一般比数据库更新速度慢很多，所以该并发问题发生的概率比较大。

如果是方案二：

​		1、多线程并发时，假设线程1先来查询缓存，并且未命中(可能缓存过期了)，然后它查到了数据库的旧数据10，并正准备更新缓存时，线程2更新了数据库的数据为20，并把缓存删除了，最后线程1执行缓存的更新，更新成了10。

​		2、此并发问题发生需要更新数据库前缓存正好不存在，并且更新数据库和删除缓存的动作并另一个线程更新缓存的动作要快，所以发生概念很低，但还是可能存在数据不一致问题。
...
缓存击穿问题也叫**热点Key**问题，就是一个被**高并发访问**并且**缓存重建业务较复杂**的key突然失效了，无数的请求访问会在瞬间给数据库带来巨大的冲击。
...
**存在问题：**现在的问题还是和之前一样，并发过来，会有多个线程查询数据库时都查询到不存在订单，会有一人多单的情况方式；所以我们还是需要加锁，但是乐观锁比较适合更新数据，而现在是插入数据，所以我们需要使用悲观锁操作。
...
​		但是以上代码还是存在问题，问题的原因在于当前方法被spring的事务控制，如果你在方法内部加锁，可能会导致当前方法事务还没有提交，但是锁已经释放。如果此时同用户对象的另一个线程也进入的方法，并且拿到了锁就会导致多次提交同个事务的问题，所以我们选择将当前方法整体包裹起来，确保事务不会出现问题。
...
public void unlock() {
    stringRedisTemplate.execute(UNLOCK_SCRIPT, List.of(KEY_PREFIX + name), ID_PREFIX + Thread.currentThread().getId());
}
...
第一个线程进来，得到了锁，手动删除锁，模拟锁超时了，其他线程会执行lua来抢锁，当第其它线程利用lua删除锁时，lua能保证他不能删除第一个线程的锁，第二个线程删除锁时，利用lua同样可以保证不会删除别人的锁，同时还能保证原子性。
...
​		可重试：利用信号量和PubSub功能实现等待、唤醒，获取锁失败的重试机制。（不是一直重试占用cpu，性能更好）

​		超时续约：利用watchDog，每隔一段时间(releaseTime / 3)，重置超时时间。（解决锁是key过期释放而不是删除key释放锁时的并发安全问题，解决超时释放问题）

​		联锁(multiLock)：设置多个独立的redis主节点，必须在所有主节点都获取重入锁成功时，才算获取锁成功。（解决主从不一致）
...
![redisson1](assets/redisson1.jpg)
...
​		原理：多个独立的redis节点，必须在所有节点都获取重入锁后，才算获取锁成功。
...
public void addSeckillVoucher(Voucher voucher) {
    ...
    voucherMap.put("beginTime", JSONUtil.toJsonStr(voucher.getBeginTime()));
    voucherMap.put("endTime", JSONUtil.toJsonStr(voucher.getEndTime()));
...
public void createVoucherOrder(VoucherOrder order) {
    int count = query().eq("user_id", order.getUserId()).eq("voucher_id", order.getVoucherId()).count();
    if (count > 0)  // 如果用户已经购买过，不允许再次下单
        log.error("用户已经购买过了！");
    boolean success = seckillVoucherService.update().setSql("stock = stock - 1")
        .eq("voucher_id", order.getVoucherId()).gt("stock", 0).update();
    if (!success)
        log.error("库存可能不足，扣减库存失败！");
    // 5、扣减成功，创建订单，保存订单到数据库中，并返回订单id
    long orderId = redisIdWorker.nextId("order");
    save(order);
}

// 通过redis使用lua脚本来完成抢购优惠卷功能，将库存和下单用户缓存到redis，再通过堵塞队列异步完成下单
public Result seckillVoucher(Long voucherId) {
    String beginTime = stringRedisTemplate.opsForHash().get(SECKILL_STOCK_KEY + voucherId, "beginTime").toString();
    String endTime = stringRedisTemplate.opsForHash().get(SECKILL_STOCK_KEY + voucherId, "endTime").toString();
    if (LocalDateTime.parse(beginTime).isAfter(LocalDateTime.now()))
        return Result.fail("秒杀尚未开始！");
    if (LocalDateTime.parse(endTime).isBefore(LocalDateTime.now()))
        return Result.fail("秒杀已经结束！");
    String userId = UserHolder.getUser().getId().toString();
    // 1、执行lua脚本
    Long r = stringRedisTemplate.execute(SECKILL_SCRIPT, List.of(), voucherId.toString(), userId);
    // 2、结果不为0，表示没有购买资格
    int result = r.intValue();
    if (result != 0)
        return Result.fail(result == 1 ? "库存不足！" : "同用户不能重复下单！");
    // 3、为0表示有购买资格，把下单信息保存到堵塞队列
    proxy = (IVoucherOrderService) AopContext.currentProxy();   // 获取代理对象
    long orderId = redisIdWorker.nextId("order");
    VoucherOrder voucherOrder = new VoucherOrder().setId(orderId).setUserId(Long.valueOf(userId))
        .setVoucherId(voucherId);
    orderTask.add(voucherOrder);
    // 4、让堵塞队列异步完成下单，把订单id直接返回
    return Result.ok(orderId);
}
```
...
![1653574849336](assets/1653574849336.png)
...
![1653575176451](assets/1653575176451.png)
...
- group：组名称
- ID：消息的ID
- min-idle-time：空闲时间，指定被消费消息中隔了[空闲时间ms]以上的未确认消息
- start 和 end：指定消息的id，表示指定要获取消息的范围，（注：- 表示最小id，+ 表示最大id，id从0开始）
...
**VoucherOrderServiceImpl的seckillvoucher方法**
...
public Result seckillVoucher(Long voucherId) {
    ...
    proxy = (IVoucherOrderService) AopContext.currentProxy();   // 获取代理对象
    String userId = UserHolder.getUser().getId().toString();
    // 1、执行lua脚本，在lua脚本中将订单信息发送到消息队列中
    Long r = stringRedisTemplate.execute(SECKILL_SCRIPT, List.of(), voucherId.toString(), userId);
    // 2、结果不为0，表示没有购买资格
    int result = r.intValue();
    if (result != 0)
        return Result.fail(result == 1 ? "库存不足！" : "同用户不能重复下单！");
    // 3、让堵塞队列异步完成下单，把订单id直接返回
    return Result.ok(orderId);
}
```

**VoucherOrderServiceImpl类中的init方法**
...
public Result likeBlog(Long id) {
    ...
    // 判断当前用户是否已经点赞过，用set保存到redis中
    Boolean isMember = stringRedisTemplate.opsForSet().isMember(key, userId.toString());
    if (BooleanUtil.isFalse(isMember)) {    // 如果未点赞，可以点赞
...
public Result likeBlog(Long id) {
    Long userId = UserHolder.getUser().getId(); // 获取登录用户
    String key = BLOG_LIKED_KEY + id;
    // 判断当前用户是否已经点赞过，用Zset保存到redis中
    Double score = stringRedisTemplate.opsForZSet().score(key, userId.toString());
    if (score == null) {    // 如果未点赞，可以点赞
        boolean success = update().setSql("liked = liked + 1").eq("id", id).update();
        if (success)
...
private void isBlogLiked(Blog blog) {
    if (UserHolder.getUser() == null)   // 用户未登录，无需查询是否点赞
        return;
    Long userId = UserHolder.getUser().getId();
...
public Result queryBlogLikes(Long id) {
    ...
    // 3.根据用户id查询用户，这里不能使用listByIds方法，因为使用的是in，不能保证顺序 WHERE id IN (5, 1) ORDER BY FIELD(id, 5, 1)
...
**注意：**数据库查询用in时，是不能保证查询出来的数据是按in括号里的顺序排列的，需要所以field来指定按指定字段的指定顺序来查询出来。如：`WHERE id IN ( 5, 3, 1) ORDER BY FIELD(id, 5, 3, 1)`
...
```java
@PutMapping("/{id}/{isFollow}")	// 关注或取关
public Result follow(@PathVariable("id") Long userId, @PathVariable("isFollow") Boolean isFollow) {
    return followService.follow(userId, isFollow);
}

@GetMapping("/or/not/{id}/")	// 判断是否关注了
...
public Result queryBlogByUserId(
    @RequestParam(value = "current", defaultValue = "1") Integer current, Long id) {
...
public Result follow(Long followUserId, Boolean isFollow) {
    ...
    if (isFollow) { // 如果是关注
        ...
        if (success)    //如果关注成功，将当前用户关注的用户的id写入redis
        ...
        if (success)    //如果取关成功，将当前用户关注的用户的id从redis移除
...
public Result followCommons(Long id) {
    ...
    if (intersect == null || intersect.isEmpty())   //如果无交集
        return Result.ok(Collections.emptyList());
    // 如果有交集，用共同关注的用户的id去查询用户信息
    List<Long> ids = intersect.stream().map(Long::valueOf).collect(Collectors.toList());
    List<UserDTO> userDTOS = userService.listByIds(ids).stream().map(user -> BeanUtil
                                                                     .copyProperties(user, UserDTO.class)).collect(Collectors.toList());
    return Result.ok(userDTOS);
}
```
...
如果在同一页出现了最小时刻一样的，还需要在新查询出的一页跳过重复的，重复的个数可以记作偏移量。如：

10 9 8 7 7 5 4 3 2 1，则第一页查询到了：10 9 8 7 7，此时的`lastId = 7`；下一页查询到的则是：7 7 5 4 3，这显然不合理，所以需要跳过最小时刻重复的，所以还需要一个偏移量来跳过重复的`offset = 2`。

总结：每一页查询为 `From min To max Limit offset size`。如上面这个例子第一页为：`From 0 To 10 Limit 0 5`，第二页为：`From 0 To 7 Limit 2 5`

注：max即为lastId，即先查询出从min到max的所有数据，再从这些数据中选出size个数据，从下标0+offset开始。

具体实现的第一步：就是我们在保存完探店笔记后，获得到当前笔记的粉丝，然后把数据推送到粉丝的redis中去。
...
public Result saveBlog(Blog blog) {
    ...
    if (!success)   //如果笔记新增失败，直接返回
        return Result.fail("新增笔记失败！");
    //如果笔记新增成功，将笔记推送给他的所有粉丝。将笔记id保存到每个粉丝的收件箱(redis的zset中)
...
1、每次查询完成后，我们要分析出查询出数据的最小时间戳，这个值会作为下一次查询的条件（作为下次查询的最大时间戳）
...
public Result queryBlogOfFollow(Long max, Integer offset) {
    ...
    long minTime = -1;  // 为tuples集合中(当前页)的最后一个元素的分数（该集合按时间降序排列）
    int count = 0;  // 偏移量，与当前页内最小的笔记创建时间（分数）相同创建时间笔记的个数
    for (ZSetOperations.TypedTuple<String> tuple : typedTuples) {
        blogIds.add(Long.valueOf(tuple.getValue()));
        long time = tuple.getScore().longValue(); // 笔记创建的时间
        if (minTime == -1)
            minTime = time; // 第一次时赋第一个创建时间为初值
        if (time == minTime)    // 如果该笔记创建时间等于最小时间
            count++;
        else {  // 如果该笔记创建时间不等于最小时间，说明当前最小时间不是最小时间
            minTime = time; // 更新最小时间
            count = 1;  // 偏移量重新开始计数
        }
    }
    // 如果上一页的最小时间等于当前页的最小时间，则偏移量需要加上上次的
    count = (minTime == max ? count + offset : count);
    // 根据id查询出blog
    ...
    ScrollResult result = new ScrollResult().setList(blogs).setOffset(count).setMinTime(minTime);
...
public Result queryShopByType(Integer typeId, Integer current, Double x, Double y) {
    // 判断是否需要根据坐标查询
    if (x == null || y == null) {   // 不需要坐标查询，按数据库查询
        Page<Shop> page = query().eq("type_id", typeId)
            .page(new Page<>(current, SystemConstants.DEFAULT_PAGE_SIZE));
        return Result.ok(page.getRecords());
    }
    // 计算分页，从redis查询，按距离排序、分页
    int from = (current - 1) * SystemConstants.DEFAULT_PAGE_SIZE;
    int end = current * SystemConstants.DEFAULT_PAGE_SIZE;
    GeoResults<RedisGeoCommands.GeoLocation<String>> results = stringRedisTemplate.opsForGeo()
        .radius(SHOP_GEO_KEY + typeId, new Circle(x, y, 5000), 
                RedisGeoCommands.GeoRadiusCommandArgs.newGeoRadiusArgs().includeDistance().limit(end));
    // 解析出店铺id和距离
    if (results == null)
        return Result.ok(Collections.emptyList());
    List<GeoResult<RedisGeoCommands.GeoLocation<String>>> list = results.getContent();
    if (list.size() <= from)    // 没有下一页了，直接返回
        return Result.ok(Collections.emptyList());
    List<Long> ids = new ArrayList<>(list.size());
    Map<String, Distance> distanceMap = new HashMap<>(list.size());
    list.stream().skip(from).forEach(result -> {    //截取from到end的部分
        String shopId = result.getContent().getName();
        ids.add(Long.valueOf(shopId));  // 获取id
        distanceMap.put(shopId, result.getDistance());  //获取距离
    });
    // 根据id查询shop
    String idStr = StrUtil.join(",", ids);
...
public Result signCount() {
    ...
    // 2.获取今天是本月的第几天
    int dayOfMonth = now.getDayOfMonth();
    // 3.获取本月截止今天为止的所有的签到记录，返回的是一个十进制的数字 BITFIELD sign:5:202203 GET u14 0
