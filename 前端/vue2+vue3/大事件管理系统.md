...
![image-20230711003405635](task2Img/image-20230711003405635.png)
...
![image-20230706205300370](task2Img/image-20230706205300370.png)

![image-20230706210947086](task2Img/image-20230706210947086.png)

![image-20230706211055251](task2Img/image-20230706211055251.png)

![image-20230706211033589](task2Img/image-20230706211033589.png)
...
![image-20230710172832242](task2Img/image-20230710172832242.png)
...
- 耗时问题：即会检查自己新提交的代码，也会检查之前已提交的代码；即每次提交，都会检查所有代码

- 历史问题：会对之前已提交代码的错误进行检查，但之前的代码可能是别人写的，自己应该只应管理好自己编写的代码就行了，不用去修改别人代码的错误。便于新的开发者专注于自己的开发



**lint-staged 配置**

通过配置它就可以只检查自己新提交的代码，不会检查之前已提交到仓库里的代码，这样即解决了耗时问题，又解决了历史代码问题
...
![image-20230710215822678](task2Img/image-20230710215822678.png)

4. 将项目需要的全局样式 和 图片文件，复制到 assets 文件夹中,  并将全局样式在main.js中引入

```jsx
import '@/task2Img/main.scss'
```
...
// vite.config.ts  添加配置  base: my-path，路由这就会加上 my-path 前缀了。此时import.meta.env.BASE_URL的值就是“/my-path”
// 在vite.config.js:  
  //plugins: [vue()],
  //base: '/my-path',
...
针对的在vue文件里会自动导入，在js文件里还是需要自己导入
...
![image-20230710225018162](task2Img/image-20230710225018162.png)
...
![image-20230711071913061](task2Img/image-20230711071913061.png)
...
.login-page {
  ...
  .bg {
    background: url('@/task2Img/logo2.png') no-repeat 60% center / 240px auto,
      url('@/task2Img/login_bg.jpg') no-repeat center / cover;
...
import avatar from '@/task2Img/default.png'
</script>
...
.layout-container {
  ...
  .el-aside {
    ...
    &__logo {
      height: 120px;
      background: url('@/task2Img/logo.png') no-repeat center / 120px auto;
    }
    .el-menu {
      border-right: none;
    }
...
![image-20230724224754131](task2Img/image-20230724224754131.png)
  ...
  <page-container title="更换头像">
    <el-row>
      <el-col :span="12">
        ...
        >
          <img v-if="imgUrl" :src="imgUrl" class="avatar" />
          <img v-else src="@/task2Img/avatar.jpg" width="278" />
        </el-upload>
        <br />
