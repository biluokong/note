...
注：a标签可以用于锚点定位，使用锚点定位需要两个部分：锚记和指向锚记的链接，有两种使用方法

- 法1：使用a标签作为锚记和链接，即使用name定位
...
- action：发送数据的地址
- method：发送数据时的请求类型，一般用 post 或 get
- autocomplete：值为 off(默认) 或 on，设置是否有历史输入提示
...
补：属性 `required` 加上后，input标签不允许空的输入
