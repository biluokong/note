...
- 360安全浏览器不会自动转换http请求为https请求，这会出现跨越，导致缓存中的数据拿不到等问题。而edge和谷歌浏览器是会自动转换的。（注：相同浏览器的不同版本的转换行为可能有所不同）
...
> 使用nginx作为web服务器，也会出现类似的情况

### vite

- 项目打包后访问首页控制台报错，提示找不到静态文件。原因：可能是生产环境使用的决定路径，需要修改为相对路径

~~~json
// vite.config.js文件
// 部署生产环境和开发环境下的URL。
// 默认情况下，vite 会假设你的应用是被部署在一个域名的根路径上
// 例如 https://www.fuyi.vip/。如果应用被部署在一个子路径上，你就需要用这个选项指定这个子路径，或使用相对路径。
// 原配置：
...
- 访问首页并登录后跳转页面显示404
  ...
  > 原因：
  >
  > - 访问 `/manage` 时，Tomcat 会尝试查找 `webapps/manage/` 目录下的文件（如 `index.html`）。
  > - 若路径是前端路由（如 `/manage/dashboard`），Tomcat 找不到对应文件，直接返回 **404**。
  
  - 方法1：在 Tomcat 的 `webapps/manage/WEB-INF/web.xml` 中添加错误转发规则，将所有 404 请求重定向到 `index.html`：
  
    ~~~xml
    <?xml version="1.0" encoding="UTF-8"?>
    <web-app xmlns="http://xmlns.jcp.org/xml/ns/javaee" version="3.1">
        <!-- 配置 404 错误转发到 index.html（支持前端 History 路由） -->
        <error-page>
  ...
  - 方法2：修改前端项目的使用模式，使用Hash模式

    > **Hash 模式**（如 `/manage/#/dashboard`）：`#` 后的部分不会被发送到服务器，因此 Tomcat 始终返回 `index.html`，由前端处理路由。
    
    ~~~js
    // 修改 createWebHistory 为 createWebHashHistory
    history: createWebHashHistory
    
    // 修改为hash模式后，使用 location.href = '/index' 的地方需要修改为下方内容，否则会用这个路径覆盖原来的路径，导致页面找不到
    location.href = '#/index'
    ~~~
